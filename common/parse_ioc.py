...
class IOCUtils(ABC):
    ...
    def __init__(self, ioc: str) -> None:
        self._IOC = ioc
        self.tld_extracted = tldextract.extract(self._IOC)
    ...
    def validate(self) -> bool:
        ...
        if self.valid:
            if validators.domain(self.url_parsed.netloc):
                self.url_type = "domain"
            if validators.ipv4(self.url_parsed.netloc):
                self.url_type = "ipv4"
            if validators.ipv6(self.url_parsed.netloc):
                self.url_type = "ipv6"
        self.type = "url"
        if not self.valid:
        ...
        return (
            {
                ...
                "is_private": self.ip_parsed.is_private,
